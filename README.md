...
- `Caddyfile`, `webdav.conf`, `prepare-root.conf`, `bootc.json`, `stunnel-postgres.conf`: standalone configs, each COPY'd to its target path. `psk.txt` is materialized at CI time from `secrets.STUNNEL_PSK` and gitignored.
