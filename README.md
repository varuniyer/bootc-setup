...
- `tailscaled.service`: full unit for the copied-in tailscaled binary (the RPM that would normally ship it is not installed). Runs under `DynamicUser` with `CAP_NET_ADMIN` and `--netfilter-mode=off` so it never touches the static nftables ruleset.
