...
bootc-based Fedora image for varuniyer.net. Runs the website, WebDAV, and a Postgres 17 instance for experiments. All three are plain system services from Fedora packages.

## How it works

- `Containerfile` builds on `quay.io/fedora/fedora-bootc:latest`. `setup.sh` installs `caddy`, `httpd`, and `postgresql17-server` from dnf and enables them as system services.
...
- `/var`: mutable and persistent. Survives auto-updates and reboots. Service state (`/var/lib/webdav`, `/var/lib/pgsql/18`, `/var/log/caddy`, `/var/lib/ssh`) lives here. `/home` and `/root` are symlinks into `/var`.
...
- `root` over ssh with sk-ed25519 keys (`PermitRootLogin prohibit-password`). The only shell account.
- `experiments` over ssh with `nologin` shell and `PermitOpen 127.0.0.1:5432` only — port-forward to Postgres, no command execution.
...
- `post-startup.{sh,service}`: boot-time, idempotent. Creates `/var` state dirs with correct ownership, runs `postgresql-18-setup initdb` and bootstraps the `experiments` role+db on first boot, refreshes Postgres configs each boot.
...
- `postgresql.conf`, `pg_hba.conf`: templates installed into `/var/lib/pgsql/18/data/` by `post-startup.sh` each boot. `pg_hba.conf` restricts TCP to `experiments` role into the `experiments` db only.
