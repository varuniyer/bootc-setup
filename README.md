...
- `provision.sh`: creates/updates secrets in GCP Secret Manager, then creates the GCP instance with `cloud-platform` scope so `post-startup.sh` can fetch them on boot. PSK is persisted once to `/var/lib/stunnel/psk.txt`; caddy hash is substituted into the Caddyfile template each boot.
